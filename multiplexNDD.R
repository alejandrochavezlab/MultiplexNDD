...
bam <- scanBam() #INSERT BAM FILE PATH HERE
...
casted[1] <- NULL
write.csv(casted, file = "",quote = F, row.names = T) #INSERT OUTPUT FILE PATH HERE
...
NBR <- #1/#2 #Number of bioreps to help with column spacing and division. Select appropriate number of replicates. INPUT REQUIRED 
...
cutoff <- colSums(casted) < 15000  # Change number as appropriate ~25% of what you hope to get is a good cut off
low.col.read.match <- vector(mode = "character", length = sum(cutoff))
for (i in 1:sum(cutoff)) { 
  #low.col.read.match[i] <- strsplit(colnames(casted)[cutoff][i], "[.]")[[1]][1]
  low.col.read.match[i] <- colnames(casted)[cutoff][i]
}
casted_filt <- casted[, !grepl(paste(low.col.read.match, collapse = "|"), colnames(casted))] # reorganize casted table so that controls are at the front of the matrix
...
for (i in 1:num_rescuers) {
  ...
  ### add necessary experiment annotations to mean var table
...
#####################################################################
######converting to numeric, accounting for p values = 1 or 0 for sumz compatibility
...
## write out all_mean_var before fitlering
...
for (value in 1:nrow(sumtable)){
  chap <- sumtable$Well[value]
  geneid <- sumtable$Gene[value]
  
  chapsubset <- all_mean_var %>% ungroup() %>% filter(Well == chap, Gene == geneid) 
  # individual test adjusted - mageck modeling
  sumtable$stouf_phigh_mag[value] <- sumz(chapsubset$mag_phigh_fdr)$p[1,1]
  sumtable$stouf_plow_mag[value] <- sumz(chapsubset$mag_plow_fdr)$p[1,1]
